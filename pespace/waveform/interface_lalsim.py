...
class WaveformLALSimulationInterface:
    ...
    The frequency-domain waveform generated by lalsimulation has the length of :math:`2^n + 1`,
    ...
    :math:`t(f)=d\\phi/df`, and use :math:`f_\\mathrm{ISCO}=1/(6^{3/2}\\pi M)` as the 
    truncation. For frequency above :math:`f_\\mathrm{ISCO}`, we set t(f)=0 adopting the 
    convention that the binary merge around t=0. If waveform model of IMRPhenomD or 
    IMRPhenomX is used, ``tiwave`` is recommended.
    """
    ...
    def __init__(
        ...
        **waveform_parameters: dict[str, Any],
        ...
        ----------
        domain : str
            Waveform domain, either 'td' for time domain or 'fd' for frequency domain.
        approximant : str
            Name of the waveform approximant (e.g., 'IMRPhenomXAS', 'SEOBNRv4').
        sampling_interval : float
            Sampling interval. If domain is 'td', this is ``dt`` in seconds.
            If domain is 'fd', this is ``df`` in Hz.
        reference_frequency : float
            Reference frequency in Hz for the waveform generation.
        minimum_frequency : float
            Minimum frequency in Hz for the waveform.
        maximum_frequency : float
            Maximum frequency in Hz for the waveform.
        skip_invalid_params : bool, optional
            If True, skip parameter sets that cause LALSimulation errors.
            If False, raise the error. Default is True.
        parameter_conversion : Callable or None, optional
            Function to convert input parameters before passing to LALSimulation.
            If None, uses default identity conversion. Default is None.
        **waveform_parameters : dict[str, Any]
            Additional waveform flags passed to LALSimulation.

        Raises
        ------
        ValueError
            If domain is not 'td' or 'fd'.

        Notes
        -----
        The ``waveform_container`` structure differs between time and frequency domains:
        
        - Time domain: dict with 'data' (Nx2 array) and 't0' (epoch) keys
    ...
    def _default_parameter_conversion(self, input_params: dict[str, float]):
        ...
        -------
        dict[str, float]
            Same as ``input_params`` (no conversion applied).
        """
        return input_params

    def _update_waveform_td(self, parameters: dict[str, float]):
        """Update time-domain waveform with new parameters.
        ...
        ------
        RuntimeError
            If LALSimulation encounters an error and ``skip_invalid_params`` is False.
        """
    ...
    def _update_waveform_fd(self, parameters: dict[str, float]):
        """Update frequency-domain waveform with new parameters.
...
def get_PN_d_phase(
    ...
    This function computes :math:`t(f) = -\\frac{1}{2\\pi}\\frac{d\\phi}{df}` using 
    ...
    frequency_samples : NDArray
        Array of frequency values in Hz at which to compute t(f).

    Returns
    -------
    NDArray
        Time-frequency correspondence t(f) in seconds. For frequencies above fISCO,
        returns 0 (adopting the convention that merger occurs at t=0).

    Notes
    -----
    The calculation uses the TaylorF2 phase approximation with PN coefficients
    from phi_0 through phi_8. The innermost stable circular orbit (ISCO)
    frequency is used as a cutoff: :math:`f_\\mathrm{ISCO}=1/(6^{3/2}\\pi M)`.
    For frequencies f<f_ISCO, the time is computed from the PN phase derivative 
    with a time shift applied so that t(f_ISCO) = 0.

    References
    ----------
    .. [1] Buonanno et al., PRD 80, 084043 (2009)
    .. [2] Blanchet, Living Rev. Relativity 17, 2 (2014)
    """
