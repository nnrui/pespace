...
class KeplerianGeocentric(OrbitModel):
    ...
    def __init__(self, arm_length:float, rotation_initial:float=0.0, revolution_initial:float=0.0)->None:
...
class KaplerianHeliocentric(OrbitModel):
    ...
    def orbit_vectors(self, time: ti.f64) -> OrbitVectorStruct:
        ...
        node1_det = self._ae_sec * vec3([sa * ca * self._sb1 - (1+sa*sa)*self._cb1, 
                                         sa * ca * self._cb1 - (1+ca*ca)*self._sb1, 
                                        -self.sqrt3 * (ca * self._cb1 + sa * self._sb1)])
        node2_det = self._ae_sec * vec3([sa * ca * self._sb2 - (1+sa*sa)*self._cb2, 
                                         sa * ca * self._cb2 - (1+ca*ca)*self._sb2, 
                                        -self.sqrt3 * (ca * self._cb2 + sa * self._sb2)])
        node3_det = self._ae_sec * vec3([sa * ca * self._sb3 - (1+sa*sa)*self._cb3, 
                                         sa * ca * self._cb3 - (1+ca*ca)*self._sb3, 
                                        -self.sqrt3 * (ca * self._cb3 + sa * self._sb3)])
        
        return OrbitVectorStruct(n1  = (node3_det - node2_det)/self.arm_length_sec,
...
available_orbit_models = {'LISA_analytic': KaplerianHeliocentric(2.5e9, 0.0, -PI/9),
