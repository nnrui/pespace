...
likelihood.parameters.update(inj_parameters)
...
if comm.rank == 0:
    ...
    parameters['ecliptic_longitude'] = 1.2*np.ones(num_tests)
    parameters['ecliptic_latitude'] = 0.8*np.ones(num_tests)
    parameters['polarization'] = np.zeros(num_tests)
