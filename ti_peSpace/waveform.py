...
default_waveform_arguments = {'neglect_waveform_errors': False,
                              }


def IMRPhenomD_h22_Amplitude_Phase_tf(frequencies, waveform, parameters, length, waveform_arguments={}, parameter_conversion=bilby.gw.conversion.convert_to_lal_binary_black_hole_parameters):
    ...
    try:
        ...
        amp, phase, tf = lalsim.SimIMRPhenomDFrequencySequenceh22AmpPhasetf(frequencies, new_parameters['mass_1'], new_parameters['mass_2'], new_parameters['chi_1'], new_parameters['chi_2'], new_parameters['luminosity_distance'], new_parameters['coalescence_time'], new_parameters['coalescence_phase'], waveform_dictionary)
    ...
    h22 = amp_data * np.exp(1j * phase_data) # NOTE whether the returned phase should include the minus
    Y22 = lal.SpinWeightedSphericalHarmonic(new_parameters['inclination'], new_parameters['coalescence_phase'], -2, 2, 2)
    Y2m2star = np.conjugate(lal.SpinWeightedSphericalHarmonic(new_parameters['inclination'], new_parameters['coalescence_phase'], -2, 2, -2))
    ...
    waveform.from_numpy({'hplus': hplus.view(dtype=np.float64).reshape(length, 2),
