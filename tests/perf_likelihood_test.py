...
import taichi as ti
ti.init(arch=ti.cuda, default_fp=ti.f64, device_memory_fraction=0.5)
...
for p in powers_of_2:
    ...
    like = Likelihood(wave_gen,
                      ...
                      data_channels,
                      psd,
                      use_gpu=False     # using gpu will have unknown error
                      )
    waveform_kwargs = dict(modes=[(2,2)], direct=True, length=len(f_array))

    st =  time.perf_counter()
    for i in range(num_tests):
        params =[parameters.iloc[i]['mass_1'],
    ...
    likelihood15.parameters.update(parameters.iloc[0])
    ...
    st =  time.perf_counter()
    for i in range(num_tests):
        likelihood15.parameters.update(parameters.iloc[i])
        ll = likelihood15.log_likelihood()
    ed = time.perf_counter()
    time_consuming = (ed - st)/num_tests
...
import json
with open('time_consuming_gpu_likelihood.json', 'w') as f:
